...
export default async function PaymentsPage() {
  const tPage = await getTranslations("Dashboard.paymentsPage");
    ...
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      <div className="rounded-2xl border border-[var(--color-primary)]/30 bg-[var(--color-primary)]/5 p-6 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-bold">{tPage("activePlan")}</h3>
            <p className="text-sm text-[var(--color-muted-fg)]">{tPage("planDesc")}</p>
          </div>
        </div>
      </div>

      <h3 className="text-lg font-semibold mb-4">{tPage("historyTitle")}</h3>
      <div className="overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)]">
        <table className="w-full text-left text-sm">
          <thead className="bg-black/5 dark:bg-white/5 border-b border-[var(--color-border)]">
            <tr>
              <th className="px-6 py-4 font-medium">{tPage("tableDate")}</th>
              <th className="px-6 py-4 font-medium">{tPage("tableAmount")}</th>
              <th className="px-6 py-4 font-medium">{tPage("tableStatus")}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--color-border)]">
            <tr>
              ...
              <td className="px-6 py-4 text-green-500 flex items-center gap-2"><CheckCircle2 className="h-4 w-4"/> {tPage("paid")}</td>
            </tr>
          </tbody>
        </table>
      </div>
